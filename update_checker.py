...
def check_update(current_version):
    try:
        if config.UPDATE_CHANNEL == "developer":
            ...
            r = requests.get(
                "https://api.github.com/repos/honzhe05/FrogCard/releases",
                headers=headers,
                timeout=5
            )
            r.raise_for_status()
            ...
            workflows_url = (
                ...
                "honzhe05/FrogCard/actions/workflows"
            )
            res = requests.get(workflows_url).json()
            workflow_url = next((s["url"] for s in res.get(
                "workflows") if s["name"] == "Build"), None)
            if not workflow_url:
                return None
            workflow_url += "/runs?per_page=1"
            res = requests.get(workflow_url).json()
            hash = res.get("workflow_runs")[0].get(
                "head_sha")[0:7].strip().lower()
            ...
            if not hash == app_version:
                if res.get("workflow_runs")[0].get("status") == "completed":
                    return {
                        ...
                        "description": (
                            f"### New commit: {hash}\n\n**Full Changelog**: "
                            f"[{app_version}...{hash}](https://github.com/"
                            f"honzhe05/FrogCard/compare/"
                            f"{app_version}...{hash})"
                        ),
                        "html_url": (
                            f"https://github.com/honzhe05/"
                            f"FrogCard/compare/{app_version}...{hash}"
                        ),
                        "apk_url": (
